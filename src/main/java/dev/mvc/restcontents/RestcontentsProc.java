...
public class RestcontentsProc implements RestcontentsProcInter {
  ...
  public int search_count(RestcontentsVO restcontentsVO) {
    int cnt = this.restcontentsDAO.search_count(restcontentsVO);
    return cnt;
  }
  ...
  public ArrayList<RestcontentsVO> list_by_restcateno_search_paging(RestcontentsVO restcontentsVO) {
    ...
    */
    int begin_of_page = (restcontentsVO.getNow_page() - 1) * Restcontents.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 10 + 10: 20
    // 3 페이지 = 20 + 10: 30
    int end_num = begin_of_page + Restcontents.RECORD_PER_PAGE;   
    /*
    1 페이지: WHERE r >= 1 AND r <= 10
    2 페이지: WHERE r >= 11 AND r <= 20
    3 페이지: WHERE r >= 21 AND r <= 30
    */
    restcontentsVO.setStart_num(start_num);
    restcontentsVO.setEnd_num(end_num);
    
    ArrayList<RestcontentsVO> list = this.restcontentsDAO.list_by_restcateno_search_paging(restcontentsVO);
    
    for (RestcontentsVO vo : list) { // 특수 문자 처리
  ...
  public String pagingBox(int restcateno, int now_page, String word, String list_file){
    RestcontentsVO restcontentsVO = new RestcontentsVO();
    restcontentsVO.setRestcateno(restcateno);
    restcontentsVO.setWord(word);
          
    int search_count = this.restcontentsDAO.search_count(restcontentsVO);  // 검색된 레코드 갯수 ->  전체 페이지 규모 파악
    int total_page = (int)(Math.ceil((double)search_count / Restcontents.RECORD_PER_PAGE)); // 전체 페이지 수 
    int total_grp = (int)(Math.ceil((double)total_page / Restcontents.PAGE_PER_BLOCK)); // 전체 그룹  수
    int now_grp = (int)(Math.ceil((double)now_page / Restcontents.PAGE_PER_BLOCK));  // 현재 그룹 번호
    
    // 1 group: 1, 2, 3 ... 9, 10
    // 2 group: 11, 12 ... 19, 20
    // 3 group: 21, 22 ... 29, 30
    int start_page = ((now_grp - 1) * Restcontents.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작  페이지  
    int end_page = (now_grp * Restcontents.PAGE_PER_BLOCK);               // 특정 그룹의 마지막 페이지   
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    int _now_page = (now_grp - 1) * Restcontents.PAGE_PER_BLOCK;  
    if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 페이지수가 11페이 이상임으로 이전 그룹으로 갈수 있는 링크 생성 
      str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&cateno="+restcateno+"'>이전</A></span>"); 
    } 
    ...
    for(int i=start_page; i<=end_page; i++){ 
      ...
      if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        str.append("<span class='span_box_1'><A href='"+list_file+"?word="+word+"&now_page="+i+"&restcateno="+restcateno+"'>"+i+"</A></span>");   
      } 
    } 
    ...
    _now_page = (now_grp * Restcontents.PAGE_PER_BLOCK)+1; //  최대 페이지수 + 1 
    if (now_grp < total_grp){ 
      str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&restcateno="+restcateno+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
  ...
  public int delete(int restcontentsno) {
    int cnt = this.restcontentsDAO.delete(restcontentsno);
    return cnt;
  }

  @Override
  public int count_by_restcateno(int restcateno) {
    int cnt = this.restcontentsDAO.count_by_restcateno(restcateno);
    return cnt;
  }
