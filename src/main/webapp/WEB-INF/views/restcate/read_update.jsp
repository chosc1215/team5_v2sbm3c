...
<DIV class='content_body'>
  <DIV id='panel_create' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <FORM name='frm_create' id='frm_create' method='POST' action='./update.do'>
      <input type="hidden" name="restcateno" value="<%=restcateVO_read.getRestcateno() %>">
      <label>카테고리 이름</label>
      <input type='text' name='name' value='<%=restcateVO_read.getName() %>' required="required" style='width: 25%;' autofocus="autofocus">
      <label>출력 순서</label>
      <input type='number' name='seqno' value='<%=restcateVO_read.getSeqno() %>' min='1' required="required" style='width: 5%;'>  
