...
<DIV class='content_body'>
  <DIV id='panel_delete' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <FORM name='frm_delete' id='frm_delete' method='POST' action='./delete.do'>
      <input type="hidden" name="notescate" value="<%=notescateVO_read.getNotescateno() %>"> <%-- 삭제할 카테고리 번호 --%>

      <c:choose>
        <c:when test="${count_by_notescate >= 1 }"> <%-- 자식 레코드가 있는 상황 --%>
          <div class="msg_warning">
            관련 자료 ${count_by_notescate } 건이 발견되었습니다.<br>
  ...
  <TABLE class='table table-hover'>
    ...
    %>
      <TR>
        ...
        <TD>
          <A href="./read_update.do?notescate=<%=notescateVO.getNotescateno() %>" title="수정"><IMG src="/notescate/images/update.png" class="icon"></A>
          <A href="./read_delete.do?notescate=<%=notescateVO.getNotescateno() %>" title="삭제"><IMG src="/notescate/images/delete.png" class="icon"></A>
        </TD>
      </TR>
    <%  
    }
