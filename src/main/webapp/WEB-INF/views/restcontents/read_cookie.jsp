...
<c:set var="file1saved" value="${restcontentsVO.file1saved }" />
...
<c:set var="size1_label" value="${restcontentsVO.size1_label }" />
...
<script type="text/javascript">
  ...
  <%-- 쇼핑 카트에 상품 추가 --%>
  function cart_ajax(restcontentsno) {
    var f = $('#frm_login');
    $('#restcontentsno', f).val(restcontentsno);  // 쇼핑카트 등록시 사용할 상품 번호를 저장.
    
    console.log('-> restcontentsno: ' + $('#restcontentsno', f).val()); 
  ...
  function cart_ajax_post() {
    var f = $('#frm_login');
    var restcontentsno = $('#restcontentsno', f).val();  // 쇼핑카트 등록시 사용할 상품 번호.
    
    var params = "";
    // params = $('#frm_login').serialize(); // 직렬화, 폼의 데이터를 키와 값의 구조로 조합
    params += 'restcontentsno=' + restcontentsno;
    params += '&${ _csrf.parameterName }=${ _csrf.token }';
...
<DIV class='title_line'>
  <A href="../restcategrp/list.do" class='title_link'>카테고리 그룹</A> > 
  <A href="../restcate/list_by_restcategrpno.do?restcategrpno=${restcategrpVO.restcategrpno }" class='title_link'>${restcategrpVO.name }</A> >
  <A href="./list_by_restcateno_search_paging.do?restcateno=${restcateVO.restcateno }" class='title_link'>${restcateVO.name }</A>
</DIV>
...
<DIV class='content_body'>
  ...
  <DIV style="text-align: right; clear: both;">  
    <form name='frm' id='frm' method='get' action='./list_by_restcateno_search.do'>
      <input type='hidden' name='restcateno' value='${restcateVO.restcateno }'>
      <c:choose>
      ...
      <c:if test="${param.word.length() > 0 }">
        <button type='button' 
                     onclick="location.href='./list_by_restcateno_search.do?restcateno=${restcateVO.restcateno}&word='">검색 취소</button>  
      </c:if>    
    </form>
  </DIV>
        ...
        <DIV style="width: 50%; float: left; margin-right: 10px;">
            <c:choose>
              <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}">
                ...
                <IMG src="/restcontents/storage/${file1saved }" style="width: 100%;"> 
              </c:when>
